...
// Only for the old Menu: Draw the backround
...
// Draw the Minetest Logo over the main menu
void drawMenuLogo(video::IVideoDriver* driver)
{
	core::dimension2d<u32> screensize = driver->getScreenSize();
	...
	if(logotexture)
	{
		...
		// The two boxes in the main menu are exactly 430 pixel heigh (guiMainMenu.cpp, drawMenu(): first rect high (270p) + second rect heigh (140p) + empty space (20p))
		// If the empty space between the top window border and the first box are not big enough for the logo, don't draw it
		s32 free_space = (screensize.Height-430)/2;
		if (free_space > logosize.Y) {
			core::rect<s32> logorect(0, 0, logosize.X, logosize.Y);
			logorect += v2s32((screensize.Width/2)-(logosize.X/2), (free_space/2)-logosize.Y/2);
			
			video::SColor bgcolor(255,50,50,50);
			//driver->draw2DRectangle(menucolor, logorect, NULL);
			
			driver->draw2DImage(logotexture, logorect,
				core::rect<s32>(core::position2d<s32>(0,0),
				core::dimension2di(logotexture->getSize())),
				NULL, NULL, true);
		}
...
// FIXME: Add a working footer with the author and version
/*
	// Draw a box at the bottom of the window
	void drawMenuFooter(video::IVideoDriver* driver)
	{
		core::dimension2d<u32> screensize = driver->getScreenSize();
		...
		{
			core::rect<s32> rect(0, screensize.Height-25, screensize.Width, screensize.Height);
...
int main(int argc, char *argv[])
{
	...
	*/
	...
	DeviceParams.Bits = 32; // TODO: Option?

	// Resolution selection
	
	// FIXED: Get fullscreen setting from the conf
	...
	core::dimension2d<int> ScreenSize;
	ScreenSize.Width = g_settings->getU16("screenW");
	ScreenSize.Height = g_settings->getU16("screenH");
	...
	DeviceParams.WindowSize = ScreenSize;
	...
	// Antialiasing (Smooth Lines)
	u16 setting_antialiasing = g_settings->getBool("antialiasing");
	if (setting_antialiasing == 0) {
		DeviceParams.AntiAlias = 0;
	}
	else if (setting_antialiasing == 1) {
		DeviceParams.AntiAlias = video::EAAM_POINT_SMOOTH; // FIXME: Quality Option
	}
	
	// vSync
	bool setting_vsync = g_settings->getBool("vsync");
	if (setting_vsync == 0) {
		DeviceParams.Vsync = false;
	}
	else if (setting_vsync == 1) {
	...
	*/
	...
	DeviceParams.EventReceiver = &receiver;
	...
	*/
	...
	skin->setColor(gui::EGDC_3D_HIGH_LIGHT, video::SColor(50,120,130,130));
	skin->setColor(gui::EGDC_3D_SHADOW, video::SColor(50,142,161,167));
		...
		try
		{
			...
			*/
			while(kill == false)
			{
				...
				menudata.antialiasing = g_settings->getBool("antialiasing");
				menudata.fullscreen = g_settings->getBool("fullscreen");
				menudata.vsync = g_settings->getBool("vsync");
				...
				float cloud_height = 1000;
				Clouds * clouds = new Clouds(smgr->getRootSceneNode(), smgr, -1,
							cloud_height, time(0));
				clouds->update(v2f(0, 0), 1.0); // We need only one update
				
				// A camera to see the clouds
				scene::ICameraSceneNode* camera = smgr->addCameraSceneNode(0,
				v3f(0,0,0), v3f(-100,60,0));
				camera->setFarValue(10000);
				
				/*
				// Get the size of the text
				core::vector2d<u32> textsize = font->getDimension(wgettext("Minetest by c55"));
				// Add a rect for the text
				core::rect<s32> textrect(10, ScreenSize.Height-25+(textsize.Y/2), textsize.X+10, ScreenSize.Height);
				// Add a text object with the author of mt
				gui::IGUIStaticText *footer_author_text = guienv->addStaticText(
					wgettext("Minetest by c55"), textrect, false, true, guiroot, -1);
				// And the same with the version
				gui::IGUIStaticText *footer_version_text = guienv->addStaticText(
					wgettext("Minetest by c55"), textrect, false, true, guiroot, -1);
				*/
				...
				while(device->run() && kill == false)
				{
					...
					clouds->step(dtime*4); // *4 otherwise the clouds moving very slowly
					//clouds->update(v2f(0, 0), 1.0);
					clouds->render();
				...
				if(newport != 0)
					port = newport;
				/* Already set in the settings menu
					g_settings->set("new_style_leaves", itos(menudata.fancy_trees));
					g_settings->set("smooth_lighting", itos(menudata.smooth_lighting));
					g_settings->set("enable_3d_clouds", itos(menudata.clouds_3d));
					g_settings->set("opaque_water", itos(menudata.opaque_water));
					g_settings->set("antialiasing", itos(menudata.antialiasing));
					g_settings->set("fullscreen", itos(menudata.fullscreen));
					g_settings->set("vsync", itos(menudata.vsync));
				*/
